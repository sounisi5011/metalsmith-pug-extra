...
function isFile(value: unknown): value is File {
  if (isObject(value)) {
    return value.hasOwnProperty('contents') && Buffer.isBuffer(value.contents);
  }
  return false;
}
...
function addFile(files: Metalsmith.Files, filename: string, contents: string): File {
  const newFile = {
...
async function render(filename: string, files: Metalsmith.Files, metalsmith: Metalsmith, options: Options) {
  ...
  const convertedText: string = pug.compile(data.contents.toString(), pugOptions)(locals);
...
export default function convert (opts: Partial<Options> = {}): Metalsmith.Plugin {
  ...
  return (files, metalsmith, done) => {
    const matchedFiles: string[] = match(Object.keys(files), options.pattern);

    Promise.all(matchedFiles.map(filename => render(filename, files, metalsmith, options)))
